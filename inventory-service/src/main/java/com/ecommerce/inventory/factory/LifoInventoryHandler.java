...
public class LifoInventoryHandler implements InventoryHandler {
    ...
    public InventoryResponse getInventory(Long productId) {
        log.debug("Fetching LIFO inventory for product: {}", productId);


        List<InventoryBatch> batches = repository.findByProductIdOrderByExpiryDateDesc(productId);
        ...
        log.debug("Found {} batches with total quantity {} for product {} (LIFO order)",
    ...
    public void updateInventory(InventoryUpdateRequest request) {
        log.debug("Updating LIFO inventory for product: {}, total quantity: {}",
        ...
        request.getBatchUpdates().forEach(batchUpdate -> {
            ...
            log.debug("Updated batch {} (LIFO): deducted {}, new quantity: {}",
                    batchId, quantityDeducted, newQuantity);
        });

        log.info("Successfully updated inventory for product: {} using LIFO strategy",
                request.getProductId());
    }

    @Override
    public String getType() {
        return InventoryStrategyType.LIFO.name();
    }

    @Override
    public InventoryStrategyType getStrategyType() {
        return InventoryStrategyType.LIFO;
    }
}